...
volatile int BPM;                   // int that holds raw Analog from A0. updated every 2mS
...
void loop()
{
 ...
 // GPS
 byte inside, i ; // GPS related variables
...
//     Serial.print(F("\n")) ;
//     Serial.print(p.y,6);        
       convert_spherical_to_Cartesian(p.x,p.y,&p.x, &p.y);
...
// delay(2000); /* Continue after short delay */
 ...
 if (client.connect(server, port))
   {
    // Serial.println("CONNECTED TO WIA") ;
    // prekey_test () ;
    root["name"] = "FALL" ;
